  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--spacing-2)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        ...
        <span style={{ fontSize: "var(--text-xl)", fontWeight: "var(--font-weight-medium)", color: "var(--text-default)", fontVariantNumeric: "tabular-nums" }}>{value} lb</span>
      </div>
...
function WLCalculatorCard() {
  ...
  // Light card matching the "Timeline to treatment" card: white ground with light
  // tide inner panels. Tinted via accent tokens so it adapts to whatever band it
  // sits on (weight-loss tide, wellness cadmium, …).
  const panel = {
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: "var(--spacing-2)", alignItems: "center" }}>
        <h3 style={{ margin: 0, fontSize: "var(--text-3xl)", fontWeight: 300, lineHeight: 1.15, color: "var(--text-default)" }}>See it before you start</h3>
        <p style={{ margin: 0, fontSize: "var(--text-base)", lineHeight: 1.5, color: "var(--fg-muted)", maxWidth: "30em" }}>Adjust targets and preview a timeline. Your clinician personalizes the final plan.</p>
      </div>
      ...
      <div className="wl-calc-grid grid" style={{ gap: "var(--spacing-5)", alignItems: "stretch", flex: 1 }}>
        ...
        <div style={Object.assign({}, panel, { display: "flex", flexDirection: "column", gap: "var(--spacing-5)" })}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--spacing-2)" }}>
            <span style={{ fontSize: "var(--text-sm)", color: "var(--fg-muted)" }}>Pace preference</span>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, background: "var(--color-white)", border: "1px solid var(--accent-border)", borderRadius: "var(--radius-lg)", padding: 4 }}>
              {paces.map((p) => (
                <button key={p.id} onClick={() => setPace(p.id)} style={{
                  ...
                  background: pace === p.id ? "var(--accent-default)" : "transparent",
                  color: pace === p.id ? "var(--color-white)" : "var(--fg-muted)",
          ...
          <div style={{
            background: "var(--accent-border)", borderRadius: "var(--radius-xl)",
            padding: "var(--spacing-4) var(--spacing-5)",
            display: "grid", gridTemplateColumns: "1fr auto", gap: "var(--spacing-4)", alignItems: "center",
          }}>
            <div>
              <div style={{ fontSize: "var(--text-xs)", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent-active)", opacity: 0.7 }}>Estimated timeline</div>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: "var(--font-weight-medium)", color: "var(--accent-active)", fontVariantNumeric: "tabular-nums" }}>{weeks} wk (~{months} mo)</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: "var(--text-xs)", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent-active)", opacity: 0.7 }}>Total change</div>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: "var(--font-weight-medium)", color: "var(--accent-active)", fontVariantNumeric: "tabular-nums" }}>−{change} lb</div>
            </div>
          </div>
          <p style={{ margin: 0, fontSize: "var(--text-xs)", lineHeight: 1.5, color: "var(--fg-muted)" }}>Projections are not guarantees. Your clinician determines eligibility, dosing, and timelines based on your individual profile.</p>
        </div>

        {/* Timeline chart */}
        <div style={Object.assign({}, panel, { display: "flex", flexDirection: "column", gap: "var(--spacing-4)" })}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "var(--text-xs)", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--fg-muted)" }}>Estimated timeline</span>
            <span style={{
              fontSize: "var(--text-xs)", fontWeight: "var(--font-weight-semibold)", color: "var(--accent-strong)",
              border: "1px solid var(--accent-border)", borderRadius: "var(--radius-4xl)",
              padding: "2px var(--spacing-3)", background: "var(--color-white)",
          ...
          <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", height: "auto", flex: 1 }} aria-label={"Projected weight from " + start + " to " + goal + " pounds"}>
            <line x1={padX} y1={padY} x2={W - padX} y2={padY} stroke="var(--accent-border)" strokeDasharray="3 4"></line>
            <line x1={padX} y1={H - padY} x2={W - padX} y2={H - padY} stroke="var(--accent-border)" strokeDasharray="3 4"></line>
            <text x={padX - 6} y={padY + 4} textAnchor="end" fontSize={fs} fill="var(--fg-muted)">{start}</text>
            <text x={padX - 6} y={H - padY + 4} textAnchor="end" fontSize={fs} fill="var(--fg-muted)">{goal}</text>
            <path d={path} fill="none" stroke="var(--accent-default)" strokeWidth="2.5" strokeLinecap="round" style={{ transition: "d 0.4s var(--ease-in-out)" }}></path>
            <circle cx={pts[0][0]} cy={pts[0][1]} r="5" fill="var(--accent-default)"></circle>
            <circle cx={goalX} cy={goalY} r="5" fill="var(--accent-default)" style={{ transition: "cx 0.4s var(--ease-in-out)" }}></circle>
            <text x={goalX} y={H - padY + 16} textAnchor="middle" fontSize={fs} fill="var(--fg-muted)" style={{ transition: "x 0.4s var(--ease-in-out)" }}>Wk {weeks}</text>
            <text x={padX} y={H - padY + 16} textAnchor="middle" fontSize={fs} fill="var(--fg-muted)">Wk 0</text>
          </svg>
          <div className="wl-calc-milestones grid" style={{ gap: "var(--spacing-3)" }}>
            {milestones.map((m) => (
              <div key={m.title} style={{
                ...
                <div style={{ fontSize: "var(--text-xs)", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent-strong)" }}>{m.wk}</div>
                <div style={{ fontSize: "var(--text-base)", fontWeight: "var(--font-weight-semibold)", color: "var(--text-default)" }}>{m.title}</div>
                <div style={{ fontSize: "var(--text-sm)", lineHeight: 1.4, color: "var(--fg-muted)" }}>{m.body}</div>
              </div>
            ))}
          </div>
        </div>
