      ...
      <div className="wl-calc-grid" style={{ display: "grid", gridTemplateColumns: "minmax(260px, 340px) 1fr", gap: "var(--spacing-5)", alignItems: "stretch", flex: 1 }}>
        ...
        <div style={Object.assign({}, panel, { display: "flex", flexDirection: "column", gap: "var(--spacing-5)" })}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, background: "rgba(38,52,34,0.30)", borderRadius: "var(--radius-lg)", padding: 4 }}>
              {paces.map((p) => (
                <button key={p.id} onClick={() => setPace(p.id)} style={{
                  ...
                  color: pace === p.id ? "#1C3049" : "rgba(255,255,255,0.85)",
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: "var(--text-xs)", letterSpacing: "0.1em", textTransform: "uppercase", color: "#1C3049", opacity: 0.7 }}>Estimated timeline</div>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: "var(--font-weight-medium)", color: "#1C3049", fontVariantNumeric: "tabular-nums" }}>{weeks} wk (~{months} mo)</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: "var(--text-xs)", letterSpacing: "0.1em", textTransform: "uppercase", color: "#1C3049", opacity: 0.7 }}>Total change</div>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: "var(--font-weight-medium)", color: "#1C3049", fontVariantNumeric: "tabular-nums" }}>−{change} lb</div>
            </div>
          </div>
        ...
        <div style={Object.assign({}, panel, { display: "flex", flexDirection: "column", gap: "var(--spacing-4)" })}>
          ...
          <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", height: "auto", flex: 1 }} aria-label={"Projected weight from " + start + " to " + goal + " pounds"}>
            ...
            <circle cx={goalX} cy={pts[24][1]} r="5" fill="var(--color-tide-300)" style={{ transition: "cx 0.4s var(--ease-in-out)" }}></circle>
...
Object.assign(window, { ChimeWeightLossSection, WLCalculatorCard, WLReveal, WLProductCarousel });
