...
function WLFaqItem({ item, open, onToggle, last }) {
    ...
    <div style={{ borderBottom: last ? "none" : "1px solid var(--accent-border)" }}>
      <button type="button" onClick={onToggle} aria-expanded={open}
        ...
        <span aria-hidden="true" style={{
          ...
          background: open ? "var(--accent-default)" : "var(--color-tide-100)",
...
function WLFaqCard({ maxWidth }) {
  ...
  return (
    <div style={{ maxWidth: maxWidth || 880, margin: "0 auto" }}>
      <div className="wl-faq-card" style={{
        ...
        {WL_FAQS.map(function (item, i) {
          return (
            <WLFaqItem key={item.q} item={item} last={i === WL_FAQS.length - 1}
              open={openIdx === i}
