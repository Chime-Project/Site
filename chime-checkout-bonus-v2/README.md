...
## Art fix + four versions (client, 2026-09-25, "combo checkout pages v2 - 4 versions.docx")

- **Art:** the "Get it delivered" truck (`images/get-it-delivered.jpg`) now carries the Chime logo instead of Remedy's "R(+)". The "All-in-one" image (`images/all-in-one.png`) is the client's Chime box (`uploads/chimeBox.jpeg`, its white ground cut out). Both are made by `uploads/combo-checkout-ref/art.py`.
- **Four versions:** this folder is the reference for them: `chime-burn-boost/`, `-3m/`, `chime-deep-belly-burn/`, `-3m/`. They are generated by `uploads/combo-checkout-ref/build.py`, which asserts every replacement.
- **If you edit this page:** rerun that build. If a replacement no longer matches, it fails and names the text, instead of shipping a half-edited version.
