/* The Chime Metabolic Reset: the in-page question carousel, the result slide
   ...
   exported for node: node chime-metabolic-reset_ind/js/reset-quiz-tests.js
...
(function (root) {
  ...
  var STORE_KEY = "chime:metabolic-reset";
  ...
  var bmi = quiz.querySelector("[data-bmi]");
  var bmiN = quiz.querySelector("[data-bmi-n]");
  ...
  function showResult() {
    ...
    if (reduced) { bmi.setAttribute("data-go", ""); return; }
    /* the marker slides 30 to 23; its label reads the BMI under it on every frame,
       so number and position can never disagree */
    bmiN.textContent = "30";
    var mark = bmiN.parentNode, rail = mark.parentNode;
    root.setTimeout(function () {
      bmi.setAttribute("data-go", "");
      var until = Date.now() + 2600;
      (function follow() {
        var x = (mark.offsetLeft + mark.offsetWidth / 2) / rail.offsetWidth; /* the dot's centre */
        var n = Math.round(18 + 17 * x);
        bmiN.textContent = String(Math.max(23, Math.min(30, n)));
        if (Date.now() < until) root.requestAnimationFrame(follow);
        else bmiN.textContent = "23";
      })();
    }, 500);
  }
