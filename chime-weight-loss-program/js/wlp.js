/* Chime Health: online weight loss program landing.
   Five behaviours, all ported from the reference by hand, no framework:
     ...
     2. filters   the Featured / GLP-1 / Combination chips
     3. priceMode the 3 month vs month to month select
   ...
   node so js/wlp-tests.js can check them without a browser. */

(function (root, factory) {
  var api = factory();
  if (typeof module === "object" && module.exports) module.exports = api;
  else root.ChimeWLP = api;
    ...
    if (medsSection && meds) {
      ...
      if (mode) {
        mode.addEventListener("change", function () {
          var three = mode.value === "3";
          Array.prototype.forEach.call(cards, function (card) {
            var el = card.querySelector("[data-price]");
            if (!el) return;
            el.textContent = fmt(three ? card.dataset.p3 : card.dataset.p1);
          });
        });
      }
