/* chime-deep-belly-burn-3m/product.html: each medication card carries its own plan-length rows; picking a row
   selects medication + length together and the card's Start button opens checkout.html?med&term.
   Prices are the doc's (combo checkout pages v2, 2026-09-25), with a 6 month plan; the 3 month plan is a straight 3 months. The selection also lands in sessionStorage "chime:checkout-selection".
...
(function () {
  ...
  function render() {
    document.querySelectorAll("[data-mp]").forEach(function (block) {
      ...
      var l2 = plan.name + (state.term === 1 ? " Monthly" : " " + state.term + " Month Plan");
