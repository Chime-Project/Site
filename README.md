...
| `/tirzepatide.html` | Tirzepatide product page (TOF) — rip of ronanrx.com/tirzepatide (client, 2026-09-14; plan in `TIRZEPATIDE-PDP-RIP-PLAN.md`), section for section on the Chime palette + Noto Serif / Quicksand: gallery (turntable clip + 4 stills), buybox with receipt, coupon and the "Already on tirzepatide?" switch door + per-dose picker, value-prop row (the four facts from the AmeriLean / police / Labor Day pages — client request), What this is, history band, statement band (label below the title), helix line, pharmacy film, dose ladder, intake copy, How it works (the reference's process, kept on request), FAQ, Explore, footer + sticky CTA. Built as a **template**: `ui_kits/pdp/` = `pdp.css` (the reference's rules re-tokened), `pdp-data.js` (one entry per product — receipts, dose panel, ladder, FAQ, explore render from it), `pdp.js` (vanilla port of the reference's nine Stimulus controllers); a second product page is a copy of the shell + a data entry. **Primary CTA is intentionally blank** (Luis). STAND-INS flagged in the head comment: the whole price ladder / $39 physician fee / FOUNDERS78 coupon are the reference's (Chime's checkout ladder is $359 / $897 / $1,794), the process claims, the media (cuts of `uploads/chime-glp1-hero-motion.mp4` + the Chime vial art), the footer note. `noindex` until signed off. `node ui_kits/pdp/pdp-tests.js` (32 checks) |
