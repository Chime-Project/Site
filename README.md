...
| `/chime-checkout-bonus-v2/product.html` | **Working copy for the redesign** of the combo offer above (client, 2026-09-21: "do a clone of this landing in a new folder"). A detached copy of `chime-checkout-bonus/` — NOT a build mode, hand-edited from here; identical to the original at clone time except `?v=` and header comments. Redesign rounds (hero + results merged, plans 4th, round 3: no members photo, results strip; round 4: the Today / With Chime report back in the hero, no hero button, plans directly under the hero; round 5: V2-style treatment selector in the hero that glides to the chosen combo's plan rows) are logged in the folder README. Same stand-ins (1 + 3 ladder, $299 value) |
